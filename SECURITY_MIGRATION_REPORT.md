...
- お客様控えPNG（非公開Storage `exhibition-receipts`、対象画像の署名付きリンクだけをお客様へ渡す）
...
## 8. 3タブと控えQRの復元（2026-09-03 追加依頼）

- 要対応・受け取り待ち・完了の3タブを復元。詳細からの状態変更は通常の注文更新としてRLS・競合検知を通して同期
- お客様控えをPNG化し、非公開Storageへ保存。旧匿名注文APIや商品マスターの公開権限は復元しない
- 署名付きリンクは画像1件に限定し、7日間を指定。QR・リンクは対象顧客だけに渡すよう画面で注意表示
- 社内備考・スタッフ名・帳合先を画像から除外。画像のパスに顧客名・電話等を含めず、注文IDと控え内容のハッシュを使用
- 同じ画像を上書きせず、注文変更時は別画像にする。以前渡したリンクはその元の期限まで有効。ダウンロード済み画像は利用者端末に残る
- バケットは非公開、PNGのみ、10MB上限。RLSは有効スタッフと対象注文の存在・閲覧可否を照合
- スタッフは画像を削除して共有を停止できる権限を持つが、注文の物理削除権限は変更しない。画像削除はStorage API経由とし、管理者による停止後に同じ画像を再発行すると旧署名URLも期限内は再利用可能な点に注意
- 実Storageでの1ピクセル画像による検証・後片付けを実施。実注文は変更せず13件、旧注文22件を維持
- 自動テスト47件、PC／スマホでのQRデコード・PNG表示・3タブ・既存のログイン／同期／印刷保持を確認
- 既存のSecurity Advisor警告は第5節のとおり継続。新しい公開関数や匿名データ取得権限は追加しない

## 9. 日付レイアウト・Slack共有確認・全注文印刷（2026-09-03 追加依頼）

- 日付欄の寸法と余白、控えQRボタンの改行を調整。「注文確定」に表記を統一
- 新規注文は要対応から開始。Slack共有確認をチェックすると完了へ移動し、解除で要対応へ戻る。既存注文の状態は一括変更しない
- `slackShared` と `slackSharedAt` を既存注文のJSON payloadに追加。既存のスタッフ限定RLS・列権限・更新時刻による競合検知をそのまま利用し、テーブル／ポリシー変更は不要
- Slackトークン・Webhook・自動投稿は追加せず、スタッフが共有済みを確認するチェックのみ。GitHubやブラウザの永続保存へ共有データは追加しない
- 旧本社共有フラグをSlack共有と解釈せず、既存状態・履歴を保持
- 一括印刷は全注文・全期間が標準。国内通常注文だけの選択を除去
- 自動テスト53件、Chrome／WebKitの6幅で5区分の入力・チェック・同期・競合・印刷保持を検証。単票と一括PDFの全ページを画像化して確認
- 本番の注文・顧客情報・画像・認証情報は変更しない。実DBへの確認はRLSと権限の読み取りのみ

## 10. Slack共有用PDFボタン・対象区分の限定（2026-09-03 追加依頼）

- 共有チェックの横に「共有」を追加。対象は現売りの後日受取／ホテル配送／指定住所配送のみ。通常注文とその場渡しは注文確定で完了
- 既存の注文書印刷処理を利用し、PDFに「未確定」などの表示は追加しない
- ボタン単体ではクラウド保存・共有済みへの更新・Slackへの送信を行わない。実際にSlackへ送った後のチェックで完了を記録
- 入力中に共有用PDFを出しても、顧客情報の新たな永続保存先を増やさない。PDFを利用者が保存した場合だけ端末に残る
- 既存の本番注文や認証・DB権限は変更しない。自動テスト55件、Chrome／WebKitの6幅、共有ボタン・入力保持・状態遷移・QR・同期・PDFの回帰を確認
