...
## 11. 後日受取のお渡し完了（2026-09-03 追加依頼）

- 後日受取はSlack共有済みでも「受け取り待ち」に残し、一覧または詳細の「お渡し済み」で完了。ホテル配送・指定住所配送は共有済みで完了のまま
- お渡し状態と日時（`delivered`・`deliveredAt`）は既存の注文payloadへ保存し、スタッフ間で同期。会計済みの記録は自動変更しない
- payloadは既存のJSONB列を使用。認証・スタッフ限定RLS・更新可能な列・更新時刻の競合検知は変更しない。秘密キーや新たな外部送信先は追加しない
- 過去に共有だけで完了となった未受取注文は受け取り待ちに表示するが、本番注文を一括更新しない。記録済みのお渡し状態は保持し、不明な過去のお渡し日時を作らない
- 通信失敗時に誤って完了としないこと、連打で重複更新しないこと、別端末での復元とお渡し取り消しを架空データで確認
- 自動テスト59件とChrome／WebKitのレイアウト・共有・同期・QR・印刷操作の回帰を確認。実SupabaseはRLS有効、匿名SELECT不可、payload更新権限とJSONB型を読み取り専用で再確認

## 12. 番号発行と段階的な注文確定（2026-09-16）

- 入力画面の「お渡し番号を発行」で、既存のスタッフ限定保存とサーバー採番を使用。番号は注文・PDF・控えへ引き継ぎ、端末から採番値を送信しない
- 途中まで準備されていた一時保存・共有確認・注文確定処理を統合。`confirmation_state` 列はDB側のトリガーで管理し、クライアントへ直接の列更新権限を付与しない
- 既存行は確定済みとして維持。新しい共有手順は `NEO TOKYO 2026` の対象注文だけに適用し、同じDBの簡単注文アプリの注文フローは変更しない
- 番号発行直後の注文もSupabaseへ保存し、閉じたり再読み込みしたりしても再開可能。未確定分は受注件数と一括印刷から除外
- RLS・匿名アクセス制限・同時更新の検知を維持。新しい関数は `SECURITY INVOKER` と固定search_pathを使用し、秘密キーを追加しない
- 実DBの架空データ検証はROLLBACKで終了し、既存の顧客注文や採番シーケンスを変更しない。ブラウザ操作はローカルの架空データを使用

## 13. 会社向けPDFへの写真添付（2026-09-17）

- 写真フォルダ選択・カメラ撮影を共有手順へ追加。JPEG等の画像だけ、最大6枚・1枚20MBまで。SVG・PDF・空ファイルは拒否
- 写真はブラウザのメモリと一時画像URLのみで保持し、注文payload・Supabase Storage・GitHub・localStorageへ追加しない。画像を端末内でJPEGへ描き直し、元写真のEXIF/GPSメタデータを持ち越さない
- 会社用の個別PDFへ写真を追加。お客様控えQR・画像・PDFと一括印刷には含めない。外部への共有はスタッフが保存したPDFを送信する操作で行う
- 新規注文・ログアウトで写真を消去し、削除した注文の写真も解放。遅い画像処理が完了しても、消去後の注文へ復活しないように世代を照合
- 写真変更後はPDFの再作成を必要とし、共有確認後の写真変更は先にチェック解除が必要。DBスキーマ・認証・権限・RLSの変更はない
- 写真添付ページは印刷時に1枚1ページへ固定し、A4内で縦横比を保って収める
