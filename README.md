...
- 「PDFを作成する」で会社控え・お客様控え・添付写真を1つのPDFにまとめます。作成後に「PDFを共有する」と「Slackに共有済み」のチェック欄を表示します。共有機能のない端末ではPDFを保存し、Slackへ添付します。PDFに「未確定」等の表示は付けません。番号発行直後から同じ番号の共有用PDFを作成でき、確定後にはQR・お客様控え画像を発行できます。
...
後日受取・ホテル配送・指定住所配送は、番号発行または一時保存後の「Slackに共有」欄で「写真フォルダから選ぶ」「カメラで撮影」を使用できます。最大6枚、1枚20MBまで。JPEG・PNG・WebPと、利用端末で読み込めるHEIC／HEIFに対応します。読めない形式はJPEG・PNGで選び直してください。カメラの起動方法は端末・ブラウザによって異なります。

写真は大きく確認・削除できます。共有用PDFと、その端末での個別の会社用注文書には、注文書の後に写真1枚につき1ページを追加します。お客様向けQR・画像・控えPDFと全注文の一括印刷には添付しません。送信確認後に写真を変える場合はチェックを外し、変更後のPDFを作成・共有し直してください。

写真は端末のメモリだけに一時保持します。GitHub・Supabase・localStorageへ保存せず、他のスタッフ端末へ同期しません。同じ注文を同じタブ内で開き直す間は保持し、再読み込み・ログアウト・新しい注文で消去します。先にPDFを保存・共有してください。保存したPDFは端末や共有先に残ります。画像は端末内でJPEGへ変換してから使用し、元写真のGPS等の付加情報は送信しません。
