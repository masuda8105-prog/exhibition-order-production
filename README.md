...
- 翌日・翌々日の後日受取には、受付番号とは別に `NEO-1`、`NEO-2`…のお渡し番号を注文確定時に発行します。全端末・全日程で共通の連番です。注文一覧・詳細・確定画面・注文書・お客様控えに表示し、番号でも検索できます。
- お渡し番号はDBの専用列・シーケンス・一意制約で管理し、端末から書き換えられません。日付や展示会でリセットせず、取消後も再利用しません。再試行等で欠番が出る場合があります。後日受取の番号入り控えは、注文確定後に印刷・発行してください。
...
- 共有チェックの横の「共有」で、注文書のPDF保存・印刷画面を開きます。PDFに「未確定」等の表示は付けません。後日受取はお渡し番号の発行が必要なので、先に注文確定し、確定後の画面から印刷します。それ以外の配送は入力画面からも印刷できます。
...
- ホテル配送・指定住所配送は、共有済みのチェックで「完了」、解除で「要対応」に戻ります。確定済みの注文は詳細画面のチェック操作で保存・同期します。
- 後日受取のお渡し前に共有チェックを外すと「要対応」へ戻ります。お渡し後は共有チェックを変更しても完了のままです。誤ってお渡し済みにした場合は、詳細の「注文の状態」を「受け取り待ち」へ変更すると、お渡し記録を取り消せます。
