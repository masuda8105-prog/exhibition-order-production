...
- 「PDFを作成する」で会社控え・添付写真を1つのPDFにまとめます。作成後に「PDFを共有する」と「Slackに共有済み」のチェック欄を表示します。共有機能のない端末ではPDFを保存し、Slackへ添付します。PDFに「未確定」等の表示は付けません。番号発行直後から同じ番号の共有用PDFを作成でき、確定後にはQR・お客様控え画像を発行できます。
