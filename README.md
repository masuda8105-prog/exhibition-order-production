...
- Slack共有欄は、現売りの「翌日・翌々日受取」「ホテル配送」「指定住所へ配送」だけに表示します。これらの新規注文は、共有チェックなしなら「要対応」です。
- 共有チェックの横の「共有」で、注文書のPDF保存・印刷画面を開きます。PDFに「未確定」等の表示は付けません。入力画面でも現在の内容を印刷でき、後日受取の受付番号は確定時にも同じものを引き継ぎます。
- PDFをSlackへ送った後、入力画面の「Slackに共有済み」にチェックして確定すると「完了」に入ります。
- 確定済みの注文は、詳細画面で同じチェックを付けるだけで保存・同期し、「完了」へ移ります。チェックを外すと「要対応」に戻ります。
- Slackへの自動投稿は行いません。「共有」はPDF保存・印刷を開くだけで、自動で共有チェックや注文確定は行いません。チェックはスタッフによる共有確認です。共有確認日時も注文データに保存します。
