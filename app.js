...
function receiptDocumentHtml(order,{customerCopy=false}={}){
  ...
  const info=[['店舗名',order.store],['電話番号',order.phone],['お客様名',order.customer||'-'],['注文区分',labelOrder(order)],['卸屋・帳合先',order.account||'-'],['担当',order.staff||state.staff?.display_name||'-'],['受け渡し',handoffLabel(order)]];if(!customerCopy)info.push(['状態',status]);
